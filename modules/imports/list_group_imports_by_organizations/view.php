...
<table  border="0" cellpadding="1px" cellspacing="1px">
	...
	<th><?php echo $CAP_update_groups?></th>
	<th><?php echo $CAP_delete_passages?></th>
   </tr>
     ...
     while ( $count_data_bylimit > $index ){
        $link = "temp_groups.php?importid=".$data_bylimit[$index]["id"]."";
	$link_update="export_groups.php?importid=".$data_bylimit[$index]["id"]."";
	$link_delete="delete_groups.php?importid=".$data_bylimit[$index]["id"]."";
	    if ( $style == "row_lite" ){
            $style="row_dark";
        }
        else{
