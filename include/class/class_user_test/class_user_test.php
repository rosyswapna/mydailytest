...
class UserTest
{
	...
	function update()
	{
		...
		{
			$strSQL = "INSERT INTO user_tests (user_id, quiz_id, test_status_id, test_date, start_time, resumed_time,used_time,total_time) VALUES ('";
		...
		elseif($this->id > 0 )
		{
			$strSQL = "UPDATE user_tests SET end_time = '".CURRENT_DATETIME."',";
			$strSQL .= " test_status_id = '".$this->test_status_id."',";
			$strSQL .= " used_time = '".$this->used_time."',";
			$strSQL .= " user_mark = (SELECT sum(user_mark) FROM user_test_rules WHERE user_test_id = '".$this->id."')";
	...
	{
		$strSQL = "SELECT id,user_id, quiz_id, test_status_id, test_date, start_time,end_time,resumed_time,used_time,total_time";
		$strSQL .= " FROM user_tests";
		$strSQL .= " WHERE id = '".$this->id."'"; //echo $strSQL;
		$rsRES = mysql_query($strSQL,$this->connection) or die(mysql_error().$strSQL);
	...
	function pause(){
		if($this->id > 0 )
		{	
			$strSQL = "UPDATE user_tests SET used_time = '".$this->used_time."', test_status_id = '".TEST_STATUS_PAUSED."'";
	...
	if($this->id > 0 ){
		//$this->get_details();
			...
			$strSQL = "UPDATE user_tests SET resumed_time = '".CURRENT_DATETIME."',test_status_id = '".TEST_STATUS_STARTED."'";
			$strSQL .= " WHERE id = '".mysql_real_escape_string(trim($this->id))."'";
			$rsRES = mysql_query($strSQL,$this->connection) or die ( mysql_error() . $strSQL );
		 	if ( mysql_affected_rows($this->connection) > 0 ) {
			  $this->id = mysql_insert_id();
...
function get_list_array_bylimit($user_id=gINVALID,$start_record = 0,$max_records = 3){
        ...
        $str_condition = "";
		if($user_id > 0){
			 $str_condition = " AND UT.user_id ='".mysql_real_escape_string($user_id)."' ";
		}
		if($this->test_status_id >0){
			 $str_condition .= " AND UT.test_status_id ='".mysql_real_escape_string($this->test_status_id)."' ";
		}		
		...
		$strSQL = "SELECT  UT.id,UT.user_id,UT.quiz_id,UT.test_status_id,UT.test_date,UT.start_time,UT.end_time,UT.resumed_time,UT.used_time, UT.total_time, QT.name as                    quiz_type_name, Q.name as quiz_name, U.username FROM user_tests UT ,users U, quizzes Q, quiz_types QT WHERE UT.user_id = U.id AND UT.quiz_id = Q.id  
			       AND Q.quiz_type_id = QT.id ".$str_condition."  ORDER BY UT.resumed_time DESC";
	...
	{
		$strSQL = "SELECT id FROM user_tests";
		$str_condition = "";
		if($this->test_status_id >0){
			 $str_condition .= "test_status_id ='".mysql_real_escape_string($this->test_status_id)."' ";
		}
