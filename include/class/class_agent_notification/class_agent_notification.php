...
function agent_password_reset(){
	$headers="";
    $strMailbody="";
    $strTo="";
    $strSubject="";
    $strSQL = "SELECT name FROM agents WHERE username = '".$this->username."'";
...
function agent_account_activation(){
	$headers="";
    $strMailbody="";
    $strTo="";
    $strSubject="";		
...
$strMailbody.="To complete your registration, you need to confirm that you received this email.<br/>";
 ...
 $strMailbody=$this->get_email_template($strMailbody); 
    //Send the mail to the Registered Agent with activation link
    mail($strTo,$strSubject,$strMailbody,$headers);
	return true;
...
function agent_password_reset_by_admin($pass){
    $headers="";
    $strMailbody="";
    $strTo="";
    $strSubject="";
	$strSQL = "SELECT username,email,name FROM agents WHERE id = '".$this->id."'";
        $rsRES = mysql_query($strSQL,$this->connection) or die(mysql_error(). $strSQL );
        if ( mysql_num_rows($rsRES) > 0 ){
